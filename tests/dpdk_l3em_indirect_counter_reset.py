...
class IPDK_L3EM_INDIRECT_COUNTER_RESET(BaseTest):
    ...
    def runTest(self):
        ...
        if not test_utils.vm_to_vm_ping_test(self.conn_obj_list[0], self.config_data['vm'][0]['remote_ip'],
            ...
            log.passed(f"The number of {2 * self.config_data['counter']['num_ping'][0]} of ping are completed")
            ...
            for j in range(len(self.config_data['counter']['index'])):
                ...
                if cnt_table_name == "ipv4_host_tbl_flow_counter_bytes":
                    if cnt1[cnt_table_name][j]['bytes'] <= 0:
                        self.result.addFailure(self, sys.exc_info())
                        self.fail(f"FAIL: The bytes counter of {cnt_table_name} before reset should be greater than 0")
                    else:
                        log.passed(f"PASS: The byte counter of {cnt_table_name} before reset is {cnt1[cnt_table_name][i]['bytes']}")
                    log.info(f"The curent counter of {cnt_table_name} after sending traffic is {cnt2[cnt_table_name][j]}")
                    if cnt2[cnt_table_name][j]['bytes'] > 0 : 
                        self.result.addFailure(self, sys.exc_info())
                        self.fail(f"FAIL: The {flow} of {cnt_table_name} is not reset to 0")
                    else:
                ...
                if cnt_table_name == "ipv4_host_tbl_flow_counter_packets":
                    if  cnt1[cnt_table_name][j]['packets'] <= 0 :
                        self.result.addFailure(self, sys.exc_info())
                        self.fail(f"FAIL: The packet counter {cnt_table_name} before reset should be greater than 0")
                    else:
                        self.passed(f"PASS: The packet counter of {cnt_table_name} before reset is {cnt1[cnt_table_name][j]['packets']}")
