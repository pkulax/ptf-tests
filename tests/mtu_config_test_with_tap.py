...
class TapPort_MTU_Config(BaseTest):
    ...
    def runTest(self):
        ...
        p4rt_ctl.p4rt_ctl_set_pipe(self.config_data['switch'], self.config_data['pb_bin'], self.config_data['p4_info'])
        ...
        log.info(f"sending packet from " + port_list[0] + " of size " + pktsize1 +"  i.e. greater than assigned MTU of " + assigned_mtu )
        ...
        try:
           ...
           if result:
               log.passed(f"Assertion verified as per difference in MTU and packet size")
           else:
               self.result.addFailure(self, sys.exc_info())
