...
from common.utils.gnmi_ctl_utils import gnmi_ctl_set_and_verify, gnmi_set_params, ip_set_ipv4,gnmi_get_params_elemt_value,gnmi_get_params_counter
...
class Tap_Link_PortCounter(BaseTest):
    ...
    def runTest(self):
        ...
        if not receive_cont_1:  
            self.result.addFailure(self, sys.exc_info())
            log.failed(f"unable to get counter of {self.config_data['port'][receive_port_id]['name']}")
        if not send_cont_1:
            self.result.addFailure(self, sys.exc_info())
            log.failed(f"unable to get counter of {self.config_data['port'][send_port_id]['name']}")
        ...
        for each in self.config_data['traffic']['in_pkt_header']['pkts_counter']:
            ...
            if update[each] in range(num, num_buffer):
                log.passed(f"{num} packets expected and {update[each]} verified on {port} {each} counter")
            else:
                log.failed(f"{num} packets expected but {update[each]} verified on {port} {each} counter")
                self.result.addFailure(self, sys.exc_info())
        ...
        for each in self.config_data['traffic']['in_pkt_header']["octets_counter"]:
            ...
            if update[each] in range(total_octets_send, octets_buffer):
                log.passed(f"{total_octets_send:} octets expected and {update[each]} verified on {port} {each} counter")
            else:
                log.failed(f"{total_octets_send} octets expected but {update[each]} verified on {port} {each} counter")
                self.result.addFailure(self, sys.exc_info())
        ...
        log.info(f"Record in-octets and in-unicast-pkts counter of TAP0 before sending traffic")
